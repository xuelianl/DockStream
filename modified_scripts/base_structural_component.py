...
class BaseStructuralComponent(BaseScoreComponent):
    ...
    def calculate_score(self, molecules: List, step=-1) -> ComponentSummary:
        ...
        score, raw_score = self._calculate_score(valid_smiles, step) # /pubhome/xli02/opt/miniconda/envs/reinvent.v3.2/lib/python3.7/site-packages/reinvent_scoring/scoring/score_components/structural/dockstream.py
    ...
    def _send_request_with_stepwize_read(self, command, data_size: int):
        with subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=None,
                              shell=True) as proc:
            # out, err = proc.communicate() # xli, binary
